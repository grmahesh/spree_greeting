module Spree
  #module Core
	...
	module GreetingFilters
      ...
      #
		Spree::Greeting.add_search_scope :price_range_any do |*opts|
			conds = opts.map {|o| Spree::Core::GreetingFilters.price_filter[:conds][o]}.reject { |c| c.nil? }
        scope = conds.shift
        conds.each do |new_scope|
          scope = scope.or(new_scope)
        end
		Spree::Greeting.joins(master: :default_price).where(scope)
      end

	def GreetingFilters.format_price(amount)
        Spree::Money.new(amount)
      end

	def GreetingFilters.price_filter
      ...
      #
	def GreetingFilters.taxons_below(taxon)
		return Spree::Core::GreetingFilters.all_taxons if taxon.nil?
        {
	...
	def GreetingFilters.all_taxons
